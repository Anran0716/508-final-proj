...
```{r nn_fucntion_use, message=FALSE, warning=FALSE}
...
```{r final_fishnet, message=FALSE, warning=FALSE}
#create final_set
...
do.call(grid.arrange,c(varList,ncol=4,top="Figure 5. Local Morans I statistics, Heroin"))
```
...
```{r regressions}
## RUN REGRESSIONS  (Spatial LOGO-CV)
...
```{r regression_error, message=FALSE, warning=FALSE}
reg.summary <- 
  rbind(
...
tracts16 <- 
  ...
  mutate(percentWhite = NumberWhites / TotalPop,
         ...
         IncContext = ifelse(Income > 27186.5, "High Income", "Low income")) %>%
  .[neighborhoods,]
...
```{r by income context, message=FALSE, warning=FALSE}
reg.summary %>% 
  filter(str_detect(Regression, "LOGO")) %>%
  st_centroid() %>%
  st_join(tracts16) %>%
...
geom_sf(data = na.omit(tracts16),
              aes(fill = raceContext)) +
               ...
               name="Race Context") +
               labs(title = "Figure 12. Test of generalizability under race context") +
...
geom_sf(data = na.omit(tracts16),
              aes(fill = IncContext)) +
               scale_fill_manual(values = c("#25CB10", "#FA7800"),
               name="Income Context") +
               labs(title = "Figure 13. Test of generalizability under income context") +
               mapTheme() + theme(legend.position="bottom")
```
...
rbind(drug_KDE_sf, drug_risk_sf) %>%
  ...
  labs(title="Figure 14. Comparison of Kernel Density and Risk Predictions",
...
ggplot() +
  ...
  labs(title = "Figure 15. Health care centres distribution under risk prediction", subtitle = "The red border indicates areas within 1/2 mile to health care centers.") +
  mapTheme()
```
